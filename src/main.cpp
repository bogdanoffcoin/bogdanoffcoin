...
#if defined(NDEBUG)
# error "Bogdanoffcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x21a2443df21ecc9e00c8b89fcc3d37be2019ca811358acb5d344a5346f6fc5fd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bogdanoffcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bogdanoffcoin Signed Message:\n";
...
{
    // Bogdanoffcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 800000 blocks, which will occur approximately every 2.5 years
    nSubsidy >>= (nHeight / 500000); // Clonecoin: 800k blocks in ~2.5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Bogdanoffcoin: 5 minutes
static const int64 nTargetSpacing = 2.5 * 60; // Bogdanoffcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bogdanoffcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bogdanoffcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x21a2443df21ecc9e00c8b89fcc3d37be2019ca811358acb5d344a5346f6fc5fd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "20/05/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1526847898;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1075308;
;
...
;
        ...
        assert(block.hashMerkleRoot == uint256("0xe3b19c9c67de99bc1828ae0cc580e2bbf3b2190a1c85351e01db8c9ba57f18f3"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Bogdanoffcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BogdanoffcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BogdanoffcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BogdanoffcoinMiner(CWallet *pwallet)
{
    printf("BogdanoffcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bogdanoffcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BogdanoffcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BogdanoffcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BogdanoffcoinMiner, pwallet));
}
